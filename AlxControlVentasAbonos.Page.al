page 50147 AlxControlVentasAbonos
{
    ApplicationArea = All;
    Caption = 'Control Ventas Abonos';
    PageType = List;
    SourceTable = AlxControlVenta;
    ...
    var
        ...
        AbonVentasQuery: Query AlxSalesCrMemoLine;
    begin
        if FactVentasQuery.Open()then begin
            while FactVentasQuery.Read()do begin
                ...
                //Customer
                if FactVentasQuery.SelltoPhoneNo <> '' then Rec."Teléfono":=FactVentasQuery.SelltoPhoneNo
                else
                    Rec."Teléfono":=FactVentasQuery.Phone_No_;
                if FactVentasQuery.SelltoEMail <> '' then Rec."Correo Electrónico":=FactVentasQuery.SelltoEMail
                else
                    Rec."Correo Electrónico":=FactVentasQuery.E_Mail;
                if FactVentasQuery.SelltoContact <> '' then Rec.Contacto:=FactVentasQuery.SelltoContact
                else
                ...
                if Rec."Importe Pendiente" = 0 then Rec.Pagado:='SI'
                else
                    Rec.Pagado:='NO';
        ...
        if AbonVentasQuery.Open()then begin
            while AbonVentasQuery.Read()do begin
                ...
                //Customer
                if AbonVentasQuery.SelltoPhoneNo <> '' then Rec."Teléfono":=AbonVentasQuery.SelltoPhoneNo
                else
                    Rec."Teléfono":=AbonVentasQuery.Phone_No_;
                if AbonVentasQuery.SelltoEMail <> '' then Rec."Correo Electrónico":=AbonVentasQuery.SelltoEMail
                else
                    Rec."Correo Electrónico":=AbonVentasQuery.E_Mail;
                if AbonVentasQuery.SelltoContact <> '' then Rec.Contacto:=AbonVentasQuery.SelltoContact
                else
