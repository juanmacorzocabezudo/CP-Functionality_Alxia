report 50155 InformeEventoImpCapituloV2
//report 50010 "Informe Evento - Imp. Capitulo"
{
    Caption = 'Informe Evento - Impresión por Capítulo';
                    ...
                    dataitem("Lineas Evento"; "Lineas Evento")
                    {
                        ...
                        begin
                            ...
                            IF "Lineas Evento".Imprime THEN BEGIN
                                ...
                                LineasEventoTMP.CantidadComensales:=CantidadComensales;
                                LineasEventoTMP.INSERT;
                                //SL >>> GAP00061
                                rCapitulos.Reset();
                                rCapitulos.SetRange(Code, LineasEventoTMP.ImprCapitulo);
                                if not rCapitulos.FindFirst()then begin
                                    rCapitulos.Init();
                                    rCapitulos.Code:=LineasEventoTMP.ImprCapitulo;
                                    rCapitulos.Descripcion:=LineasEventoTMP.DescripCapitulo;
                                    rCapitulos.Total:=Importe;
                                    rCapitulos.CantidadComensales:=LineasEventoTMP.CantidadComensales;
                                    rCapitulos.Insert();
                                end
                                else
                                begin
                                    rCapitulos.Total:=rCapitulos.Total + Importe;
                                    rCapitulos.Modify();
                                end;
                            //SL <<< GAP00061
                            END;
                            /*  case "Lineas Evento".Tipo of
                                 "Lineas Evento".Tipo::Adulto:
                                     begin
                                         TotalImporteAdulto := TotalImporteAdulto + Importe;
                                         ContarTotales := 1;
                                     end;
                                 "Lineas Evento".Tipo::"Niño":
                                     begin
                        ...
                        begin
                            ...
                            rCapitulos.Reset();
                            rCapitulos.DeleteAll();
                            CLEAR(GrupoRegIVANeg);
                            // GAP00045 >>>
                            /*  case "Lineas Evento".Tipo of
                                 "Lineas Evento".Tipo::Adulto:
                                     LineasEventoTMP.Orden := 1;
                                 "Lineas Evento".Tipo::"Niño":
                                     LineasEventoTMP.Orden := 2;
                             end; */
                            // GAP00045 <<<
                            if not ImprimirComentarios then SetFilter("No.", '<>%1', '');
                        //SL Capitulos y Sub capitulos >>> GAP00061
                        //EventLns.RESET;
                        //EventLns.SETRANGE("Codigo Evento", Evento."Codigo Evento");
                        //if EventLns.FindFirst() then
                        //SL Capitulos y Sub capitulos <<< GAP00061
                        end;
                    }
 ...
 0: begin
                    ...
                    dataitem(LineasEventoTemp; Integer)
                    {
                        ...
                        column(LineasEvento_Cantidad; LineasEventoTMP.CantidadComensales)
                        {
                        }
                        ...
                        column(LineasEvento_Capitulo; LineasEventoTMP.ImprCapitulo)
                        {
                        }
                        column(LineasEvento_CapituloDesc; LineasEventoTMP.DescripCapitulo)
                        {
                        }
                    ...
                    dataitem(TotalMenu; Integer)
                    {
                        ...
                        begin
                            rCapitulos.Reset();
                            if rCapitulos.FindFirst()then;
                            repeat if rCapitulos.CantidadComensales < 0 then importeDescComensal:=Abs(rCapitulos.Total)
                                else
                                    totalComensales:=totalComensales + rCapitulos.CantidadComensales;
                            until rCapitulos.Next() = 0;
                            importeExtraComensal:=TotalImporte / totalComensales;
                            importeDescComensal:=importeDescComensal / totalComensales;
                            rCapitulos.Reset();
                            SetRange(Number, 1, rCapitulos.Count);
                        end;
                        trigger OnAfterGetRecord()
                        begin
                            //SL >>> GAP00061
                            if Number = 1 then rCapitulos.FindFirst()
                            else
                                rCapitulos.Next();
                            if rCapitulos.Code <> '' then begin
                                TotalMenuDesc:='TOTAL ' + rCapitulos.Code;
                                if rCapitulos.CantidadComensales <> 0 then TotalMenuImp:=((rCapitulos.Total / rCapitulos.CantidadComensales) + (importeExtraComensal))
                                else
                                    TotalMenuImp:=rCapitulos.Total;
                                TotalMenuIVA:=StrSubstNo('€ + %1 IVA', Format("Lineas Evento"."% IVA") + '%');
                            end;
                        //SL <<< GAP00061
                        end;
                    }
    ...
    rendering
    {
        layout(ImpresionPorCapitulo)
        {
            Type = RDLC;
            Caption = 'Informe Evento - Impresión por capítulo';
            LayoutFile = './src/Layout/50010.InformeEventoImpresionPorCapitulo2609.rdl';
            Summary = './src/Layout/50010.InformeEventoImpresionPorCapitulo2609.rdl';
        }
    }
    labels
    {
    ...
    EventLnsTemp: Record "Lineas Evento" temporary; //SL >>> GAP00061
    EventLns: Record "Lineas Evento"; //SL >>> GAP00061
    //SL >>> GAP00061
    rCapitulos: Record AlxImpresionCapitulos temporary;
    importeExtraComensal: Decimal;
    importeDescComensal: Decimal;
