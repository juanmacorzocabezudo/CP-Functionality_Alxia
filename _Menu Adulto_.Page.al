page 50007 "Menu Adulto"
{
    ...
    //   Fecha: 22-02-2017
    //   Técnico: JAB
    //   Presupuesto: Proyecto I003929 - Informe Producción
    //   Modificación: Incluir un nuevo botón que sea imprimir la hoja de producción
    // 
    //   Etiqueta: ADV001
    // -----------------------------------------------------
    AutoSplitKey = true;
        ...
        area(content)
        {
            repeater(Group)
            {
                ...
                field(DescripCapitulo; Rec.DescripCapitulo)
                {
                    ...
                    ToolTip = 'Como aparecera en el reporte.';
                }
                field(CantidadComensales; Rec.CantidadComensales)
                {
                    ApplicationArea = All;
                    Visible = true;
                }
        ...
        area(Processing)
        {
            ...
            action(Imprimir)
            {
                ...
                begin
                    ...
                    gr_imprimirreport.SETTABLEVIEW(gr_lineaevento);
            ...
            action(AGRALAInfoCalidadCabecera)
            {
                ...
                begin
                    ...
                    rlComponentesEvento.SETRANGE(Tipo, rlComponentesEvento.Tipo::Adulto);
    ...
    /*   trigger OnClosePage()
      ...
      begin
          ...
          if recEvento.FindFirst() then
              if (recEvento.Estado = recEvento.Estado::Presupuesto) or (recEvento.Estado = recEvento.Estado::Aceptado) then
                  if (recEvento."Total Adultos" <> 0) or (recEvento."Total Ninos" <> 0) then begin
                      recEvento.gfu_CalculoCostesPrecios();
                      CurrPage.UPDATE(true);
                  end;
      end; */
