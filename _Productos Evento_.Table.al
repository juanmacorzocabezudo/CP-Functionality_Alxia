table 50016 "Productos Evento"
{
    ...
    begin
        IF(xRec.Cantidad <> Rec.Cantidad) OR (xRec."Tipo Margen" <> Rec."Tipo Margen") OR (xRec."Valor Margen" <> Rec."Valor Margen")THEN BEGIN
            gt_producto.GET(Producto);
            //++ KR 29/01/21
            IF gt_producto."Standard Cost" <> 0 THEN "Coste Unitario":=gt_producto."Standard Cost"
            ELSE
                //-- KR
            ...
            if RecComp.FindFirst()then repeat costeTotal:=costeTotal + RecComp.CosteUnitario;
    ...
    begin
        ...
        if recPE.Cantidad <> 0 then IF RecPE."Tipo Margen" = RecPE."Tipo Margen"::Porcentaje THEN RecPE."Precio Propuesto":=(RecPE."Coste Total" * (1 + RecPE."Valor Margen" / 100)) / RecPE.Cantidad
                ...
                IF Rec.Tipo = Rec.Tipo::Otros THEN BEGIN
                    ...
                    IF lt_bom.FINDSET THEN BEGIN
                        ...
                        REPEAT lt_lincom.INIT;
                            ...
                            ELSE
                            BEGIN
                                IF Rec.Cantidad <> 0 THEN lt_lincom.VALIDATE("Quantity per", lt_lincom."Cantidad por Lote" / Rec.Cantidad)
                                ELSE
 ...
 ((NOT(FieldNum IN[AsmHeader.FIELDNO("Item No."), AsmHeader.FIELDNO("Variant Code"), AsmHeader.FIELDNO("Location Code"), AsmHeader.FIELDNO("Starting Date"), AsmHeader.FIELDNO(Quantity), AsmHeader.FIELDNO("Unit of Measure Code"), AsmHeader.FIELDNO("Quantity to Assemble"), AsmHeader.FIELDNO("Dimension Set ID")])) AND (NOT ReplaceLinesFromBOM))THEN EXIT;
    ...
    */
    ...
    local procedure CalcEarliestDueDate(AsmHeader: Record "Assembly Header"; EarliestStartingDate: Date)EarliestDueDate: Date var
        ReqLine: Record 246;
    ...
    local procedure CompruebaSiHayComponentes(): Boolean var
        lt_lincom: Record "Componentes Evento";
        lt_linprod: Record "Productos Evento";
    begin
        lt_lincom.RESET;
        ...
        lt_lincom.SETRANGE(lt_lincom."Linea Evento", Linea);
        lt_lincom.SETRANGE(lt_lincom."Parent Item No.", Rec."Codigo Producto");
        if lt_lincom.FindFirst()then EXIT(true)
        else
            exit(false);
    end;
    ...
    begin
        if Rec.Tipo = Rec.tipo::Pan then begin
            ...
            if rItem."Assembly BOM" = false then begin
                ...
                lt_lincom.SETRANGE(lt_lincom."Linea Evento", Linea);
                lt_lincom.SETRANGE(lt_lincom."Parent Item No.", Rec."Codigo Producto");
                IF lt_lincom.FINDFIRST THEN REPEAT IF Rec.Cantidad <> 0 THEN BEGIN
    ...
    procedure GetCantLote(BOM: Code[20]; Item: Code[20]): Decimal var
        lt_bom: Record 90;
    begin
        lt_bom.reset;
        lt_bom.SETRANGE(lt_bom."Parent Item No.", BOM);
        lt_bom.SETRANGE(lt_bom."No.", Item);
        if lt_bom.FINDFIRST then exit(lt_bom."Cantidad por Lote")
        else
            exit(0);
    end;
}
