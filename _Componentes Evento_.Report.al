report 50018 "Componentes Evento"
{
    ApplicationArea = All;
    Caption = 'Componentes Evento';
    UsageCategory = Administration;
    RDLCLayout = './src/Layout/ComponentesEvento.rdl';
            ...
            dataitem(CopyLoop;2000000026)
            {
                ...
                dataitem("Lineas Evento"; "Lineas Evento")
                {
                    ...
                    begin
                        ...
                        IF bolMostrarRecursos THEN RecComponentes.SETFILTER(RecComponentes.Type, '%1|%2', RecComponentes.Type::Item, RecComponentes.Type::Resource)
                        ELSE
                            RecComponentes.SETRANGE(RecComponentes.Type, RecComponentes.Type::Item);
                        IF RecComponentes.FINDSET THEN REPEAT intNumLinea+=10000;
                ...
                dataitem("Productos Evento"; "Productos Evento")
                {
                    ...
                    begin
                        ...
                        CASE "Productos Evento".Tipo OF "Productos Evento".Tipo::Menaje: BEGIN
                            RecComponentesTemp."Linea Evento":="Productos Evento".Linea * 100;
                        END;
                        "Productos Evento".Tipo::Suplementos: BEGIN
                            RecComponentesTemp."Linea Evento":="Productos Evento".Linea * 200;
                        END;
                        "Productos Evento".Tipo::Pan: BEGIN
                ...
                dataitem("Temp Componentes Evento";2000000026)
                {
                    ...
                    trigger OnAfterGetRecord()
                    begin
                        IF "Temp Componentes Evento".Number = 1 THEN RecComponentesTemp.FINDFIRST
                        ELSE
                            RecComponentesTemp.NEXT;
                        //RecComponentesTemp.CALCFIELDS("Assembly BOM");
                        //IF RecComponentesTemp."Assembly BOM" THEN BEGIN
                        CASE RecComponentesTemp."Importancia en Coste" OF 1: txtColorFondo:='#FFC8A2';
                        2: txtColorFondo:='#FFFFB5';
                        3: txtColorFondo:='#D4F0F0';
                        4: txtColorFondo:='#B6CFB6';
                        5: txtColorFondo:='#CEDBDA';
                        ELSE
    ...
    begin
        ...
        IF bolMostrarRecursos THEN RecComponenteHijo.SETFILTER(RecComponenteHijo.Type, '%1|%2', RecComponenteHijo.Type::Item, RecComponenteHijo.Type::Resource)
        ELSE
            RecComponenteHijo.SETRANGE(RecComponenteHijo.Type, RecComponenteHijo.Type::Item);
        IF RecComponenteHijo.FINDSET THEN REPEAT intNumLinea+=10000;
