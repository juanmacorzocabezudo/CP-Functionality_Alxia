page 50090 "AlxO365Activities"
{
    ...
    layout
    {
        area(content)
        {
            ...
            cuegroup("Ongoing Sales")
            {
                ...
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Completamente Reservado de Stock';
                ...
                actions
                {
                    action(CreateIncomingDocumentFromCamera)
                    {
                        ...
                        var
                            ...
                            PictureName: Text;
                        begin
                            if not Camera.GetPicture(InStr, PictureName)then exit;
            ...
            cuegroup("Incoming Documents")
            {
                ...
                field("Awaiting Verfication"; Rec."Inc. Doc. Awaiting Verfication")
                {
                    ...
                    begin
                        if OCRServiceSetup.Get()then if OCRServiceSetup.Enabled then HyperLink(OCRServiceSetup."Sign-in URL");
                    end;
                }
            }
    ...
    begin
        ...
        if IntegrationSynchJobErrors.IsEmpty()then IntegrationErrorsCue:='Favorable'
        else
            IntegrationErrorsCue:='Unfavorable';
        CRMIntegrationRecord.SetRange(Skipped, true);
        if CRMIntegrationRecord.IsEmpty()then CoupledErrorsCue:='Favorable'
        else
    ...
    var
        TaskId: Integer;
    begin
        if PBTList.Count() > 0 then foreach TaskId in PBTList.Keys do begin
    ...
    begin
        ...
        if not PBTList.ContainsKey(TaskId)then exit;
    ...
    begin
        ...
        if PBTList.ContainsKey(TaskId) or (TaskIdCalculateCue = TaskId)then begin
            ...
            if PBTList.ContainsKey(TaskId)then begin
                PBTList.Remove(TaskId);
                if PBTList.Count() = 0 then Rec."Last Date/Time Modified":=CurrentDateTime;
            end;
            Rec.Modify(true);
            Commit();
        end end;
    ...
    begin
        ...
        if ICSetup.Get()then ShowIntercompanyActivities:=(ICSetup."IC Partner Code" <> '') and ((Rec."IC Inbox Transactions" <> 0) or (Rec."IC Outbox Transactions" <> 0));
    end;
    local procedure StartWhatIsNewTour(hasTourCompleted: Boolean): Boolean var
        O365UserTours: Record "User Tours";
        TourID: Integer;
    begin
        TourID:=O365GettingStartedMgt.GetWhatIsNewTourID();
        if O365UserTours.AlreadyCompleted(TourID)then exit(false);
  ...
  */
    ...
    local procedure PrepareUserTours(): Boolean begin
