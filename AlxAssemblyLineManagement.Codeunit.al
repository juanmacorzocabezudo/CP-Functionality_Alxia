codeunit 50006 AlxAssemblyLineManagement
{
    ...
    var Text001: Label 'Do you want to update the %1 on the lines?';
    ...
    local procedure LinesExist(AsmHeader: Record 900): Boolean var
    ...
    begin
        SetLinkToLines(AsmHeader, AssemblyLine);
        IF AssemblyLine.FINDSET THEN REPEAT AssemblyLine.UpdateAvailWarning;
                AssemblyLine.MODIFY;
            UNTIL AssemblyLine.NEXT = 0;
    end;
 ...
 ((NOT(FieldNum IN[AsmHeader.FIELDNO("Item No."), AsmHeader.FIELDNO("Variant Code"), AsmHeader.FIELDNO("Location Code"), AsmHeader.FIELDNO("Starting Date"), AsmHeader.FIELDNO(Quantity), AsmHeader.FIELDNO("Unit of Measure Code"), AsmHeader.FIELDNO("Quantity to Assemble"), AsmHeader.FIELDNO("Dimension Set ID")])) AND (NOT ReplaceLinesFromBOM))THEN EXIT;
    ...
    local procedure PreCheckAndConfirmUpdate(AsmHeader: Record 900; OldAsmHeader: Record 900; FieldNum: Integer; var ReplaceLinesFromBOM: Boolean; var TempAssemblyLine: Record 901 temporary; var UpdateDueDate: Boolean; var UpdateLocation: Boolean; var UpdateQuantity: Boolean; var UpdateUOM: Boolean; var UpdateQtyToConsume: Boolean; var UpdateDimension: Boolean): Boolean begin
        ...
        CASE FieldNum OF AsmHeader.FIELDNO("Item No."): BEGIN
        ...
        END;
        AsmHeader.FIELDNO("Variant Code"): UpdateDueDate:=TRUE;
        AsmHeader.FIELDNO("Location Code"): BEGIN
            UpdateDueDate:=TRUE;
            ...
            END;
        END;
        AsmHeader.FIELDNO("Starting Date"): UpdateDueDate:=TRUE;
        AsmHeader.FIELDNO(Quantity): IF AsmHeader.Quantity <> OldAsmHeader.Quantity THEN BEGIN
                UpdateQuantity:=TRUE;
                UpdateQtyToConsume:=TRUE;
            END;
        AsmHeader.FIELDNO("Unit of Measure Code"): IF AsmHeader."Unit of Measure Code" <> OldAsmHeader."Unit of Measure Code" THEN UpdateUOM:=TRUE;
        AsmHeader.FIELDNO("Quantity to Assemble"): UpdateQtyToConsume:=TRUE;
        AsmHeader.FIELDNO("Dimension Set ID"): IF AsmHeader."Dimension Set ID" <> OldAsmHeader."Dimension Set ID" THEN BEGIN
    ...
    procedure CopyAssemblyData(FromAssemblyHeader: Record 900; var ToAssemblyHeader: Record 900; var ToAssemblyLine: Record 901)NoOfLinesInserted: Integer var
        AssemblyLine: Record 901;
    begin
    ...
    procedure ShowAvailability(ShowPageEvenIfEnoughComponentsAvailable: Boolean; var TmpAssemblyHeader: Record 900; var TempAssemblyLine: Record 901 temporary)Rollback: Boolean var
        Item: Record 27;
        TempAssemblyLine2: Record 901 temporary;
    ...
    local procedure CalcAvailToAssemble(AssemblyLine: Record 901; AsmHeader: Record 900; var LineAvailabilityDate: Date)LineAbleToAssemble: Decimal var
        Item: Record 27;
        GrossRequirement: Decimal;
    ...
    procedure CompletelyPicked(AsmHeader: Record 900): Boolean var
        AssemblyLine: Record 901;
    begin
        SetLinkToItemLines(AsmHeader, AssemblyLine);
        IF AssemblyLine.FIND('-')THEN REPEAT IF NOT AssemblyLine.CompletelyPicked THEN EXIT(FALSE);
            UNTIL AssemblyLine.NEXT = 0;
        EXIT(TRUE);
    end;
