table 50002 "Lineas Evento"
{
        ...
        field(25; Bocados; Integer)
        {
            ...
            begin
                ...
                IF(Tipo = Tipo::Otros) AND (Rec.Bocados <> 0) AND (Rec.Bocados <> xRec.Bocados) AND (xRec.Bocados <> 0)THEN BEGIN
                    ...
                    lt_lincom.SETRANGE(lt_lincom."Parent Item No.", Rec."No.");
                    //ADV004 Fin
                    IF lt_lincom.FINDSET THEN REPEAT lt_lincom.Bocados:=lt_lincom.Bocados * (Rec.Bocados / xRec.Bocados);
    ...
    var Text10000: Label 'No puede asignar cantidad 0 a la linea ya que tiene componentes. ';
                ...
                IF Rec.Tipo = Rec.Tipo::Otros THEN BEGIN
                    ...
                    IF lt_bom.FINDSET THEN BEGIN
                        ...
                        REPEAT lt_lincom.INIT;
                            ...
                            IF(lt_lincom.Type = lt_lincom.Type::Resource) AND (lt_lincom."Resource Usage Type" = lt_lincom."Resource Usage Type"::Fixed)THEN lt_lincom.VALIDATE("Quantity per", lt_lincom."Cantidad por Lote")
                            ELSE
                            BEGIN
    ...
    local procedure lfu_TieneComponentes()TieneLineas: Boolean var
        lt_lincom: Record "Componentes Evento";
        cantidadComp: Decimal;
    begin
        TieneLineas:=FALSE;
    ...
    begin
        ...
        IF Tipo = Rec.Tipo::Adulto THEN BEGIN
            IF Rcd_Evento."Total Adultos" <> 0 THEN ImportePorPersona:=ROUND(Importe / Rcd_Evento."Total Adultos", GLSetup."Amount Rounding Precision")
            ELSE
                ImportePorPersona:=0;
        END
        ELSE IF Tipo = Rec.Tipo::Niño THEN BEGIN
                IF Rcd_Evento."Total Ninos" <> 0 THEN ImportePorPersona:=ROUND(Importe / Rcd_Evento."Total Ninos", GLSetup."Amount Rounding Precision")
                ELSE
                    ImportePorPersona:=0;
            END
            ELSE IF Tipo = Rec.Tipo::Otros THEN BEGIN
                    IF(Rcd_Evento."Total Adultos" + Rcd_Evento."Total Ninos") <> 0 THEN ImportePorPersona:=ROUND(Importe / (Rcd_Evento."Total Adultos" + Rcd_Evento."Total Ninos"), GLSetup."Amount Rounding Precision")
                    ELSE
                        ImportePorPersona:=0;
                END;
    end;
                ...
                CASE gt_Escalados.TipoTramo OF 0: BEGIN
                    ...
                    IF gt_Escalados.LoteReceta <= gd_CantidadCabecera THEN IF gt_Escalados.CalculoProporcional THEN BEGIN
                            ...
                            IF pt_LineaComponente.Type = gt_Escalados.Tipo::Recurso THEN BEGIN
                                lt_Componente.RESET;
                                IF lt_Componente.GET(gt_Escalados.NumeroLM, gt_Escalados.NumeroLinea)THEN IF lt_Componente.TipoRecurso = lt_Componente.TipoRecurso::Machine THEN pt_LineaComponente."Cantidad por Lote":=ROUND(pt_LineaComponente."Cantidad por Lote", 1, '>')END;
                            // Fin ADV002
                            li_CantTramoAnterior:=pt_LineaComponente."Cantidad por Lote" END
                        ELSE
                        BEGIN
                            //++AGRALA
                    ...
                    ELSE // Compruebo si el resultado del cálculo es mayor a la cantidad configurada en el tramo superior
                        IF li_CantTramoAnterior <> 0 THEN IF gt_Escalados.CantidadLoteReceta <= li_CantTramoAnterior THEN BEGIN
                                pt_LineaComponente."Cantidad por Lote":=gt_Escalados.CantidadLoteReceta;
                                pt_LineaComponente.CantidadEscalado:=li_CantTramoAnterior END;
                END;
                1: BEGIN
                    lb_ExisteEscaladoSuperior:=TRUE;
    ...
    begin
        ...
        IF lt_Item."Assembly BOM" THEN BEGIN
            ...
            lt_lincom.SETRANGE(lt_lincom."Codigo Evento", Rec."Codigo Evento");
            lt_lincom.SETRANGE(lt_lincom."Linea Evento", Rec.Linea);
            IF lt_lincom.FINDSET THEN REPEAT LineNo:=LineNo + 10000;
                    AsmLine.INIT;
                    AsmLine."Document Type":=AsmHeader."Document Type";
                    AsmLine.VALIDATE("Document No.", AsmHeader."No.");
                    AsmLine.VALIDATE(Type, lt_lincom.Type);
                    ...
                    //inicio si es cocktel y se mete como menu, cojo cantidades totales en lugar de cantidades unitarias.
                    IF(lt_Item."Lote Receta" = 1) AND (Rec.Tipo <> Rec.Tipo::Otros)THEN AsmLine.VALIDATE(AsmLine."Quantity per", lt_lincom."Cantidad por Lote")
                    ELSE
                        AsmLine.VALIDATE("Quantity per", lt_lincom."Cantidad por Lote" / Rec.Cantidad);
                    AsmLine.MODIFY;
                UNTIL lt_lincom.NEXT = 0;
 ...
 ((NOT(FieldNum IN[AsmHeader.FIELDNO("Item No."), AsmHeader.FIELDNO("Variant Code"), AsmHeader.FIELDNO("Location Code"), AsmHeader.FIELDNO("Starting Date"), AsmHeader.FIELDNO(Quantity), AsmHeader.FIELDNO("Unit of Measure Code"), AsmHeader.FIELDNO("Quantity to Assemble"), AsmHeader.FIELDNO("Dimension Set ID")])) AND (NOT ReplaceLinesFromBOM))THEN EXIT;
        ...
        IF ReplaceLinesFromBOM THEN BEGIN
            ...
            IF NOT((AsmHeader."Quantity (Base)" = 0) OR (AsmHeader."Item No." = ''))THEN BEGIN // condition to replace asm lines
                ...
                IF BomComponent.FINDSET THEN REPEAT InsertAsmLine(AsmHeader, TempAssemblyLine, TRUE);
        ...
        IF NOT ReplaceLinesFromBOM THEN IF TempAssemblyLine.FIND('-')THEN REPEAT TempCurrAsmLine:=TempAssemblyLine;
        ...
        */
        ...
        END;
        IF TempAssemblyLine.FIND('-')THEN REPEAT IF NOT ReplaceLinesFromBOM THEN AssemblyLine.GET(TempAssemblyLine."Document Type", TempAssemblyLine."Document No.", TempAssemblyLine."Line No.");
                AssemblyLine:=TempAssemblyLine;
                IF ReplaceLinesFromBOM THEN AssemblyLine.INSERT(TRUE)
                ELSE
    ...
    procedure CopyAssemblyData(FromAssemblyHeader: Record "Assembly Header"; var ToAssemblyHeader: Record "Assembly Header"; var ToAssemblyLine: Record "Assembly Line")NoOfLinesInserted: Integer var
        AssemblyLine: Record "Assembly Line";
    begin
        ...
        IF AssemblyLine.FIND('-')THEN REPEAT ToAssemblyLine:=AssemblyLine;
    ...
    local procedure PreCheckAndConfirmUpdate(AsmHeader: Record "Assembly Header"; OldAsmHeader: Record "Assembly Header"; FieldNum: Integer; var ReplaceLinesFromBOM: Boolean; var TempAssemblyLine: Record "Assembly Line" temporary; var UpdateDueDate: Boolean; var UpdateLocation: Boolean; var UpdateQuantity: Boolean; var UpdateUOM: Boolean; var UpdateQtyToConsume: Boolean; var UpdateDimension: Boolean): Boolean begin
        ...
        CASE FieldNum OF AsmHeader.FIELDNO(AsmHeader."Item No."): BEGIN
        ...
        END;
        AsmHeader.FIELDNO(AsmHeader."Variant Code"): UpdateDueDate:=TRUE;
        AsmHeader.FIELDNO(AsmHeader."Location Code"): BEGIN
            UpdateDueDate:=TRUE;
            IF AsmHeader."Location Code" <> OldAsmHeader."Location Code" THEN BEGIN
                ...
                IF NOT TempAssemblyLine.ISEMPTY THEN //-- #9627
                    IF SkipConfirmUpdate THEN UpdateLocation:=TRUE
                    ELSE
                    BEGIN
                        /************************ TEXTO ORIGINAL ************************/
            ...
            END;
        END;
        AsmHeader.FIELDNO(AsmHeader."Starting Date"): UpdateDueDate:=TRUE;
        AsmHeader.FIELDNO(AsmHeader.Quantity): IF AsmHeader.Quantity <> OldAsmHeader.Quantity THEN BEGIN
                UpdateQuantity:=TRUE;
                UpdateQtyToConsume:=TRUE;
            END;
        AsmHeader.FIELDNO(AsmHeader."Unit of Measure Code"): IF AsmHeader."Unit of Measure Code" <> OldAsmHeader."Unit of Measure Code" THEN UpdateUOM:=TRUE;
        AsmHeader.FIELDNO(AsmHeader."Quantity to Assemble"): UpdateQtyToConsume:=TRUE;
        AsmHeader.FIELDNO(AsmHeader."Dimension Set ID"): IF AsmHeader."Dimension Set ID" <> OldAsmHeader."Dimension Set ID" THEN BEGIN
        ...
        NOT ReplaceLinesFromBOM THEN EXIT(TRUE);
    end;
    ...
    begin
        ...
        IF UpdateQuantity THEN BEGIN
            ...
            // Fin ADV001
            IF AssemblyLine.FixedUsage THEN AssemblyLine.VALIDATE(Quantity)
            ELSE
        ...
        IF UpdateUOM THEN BEGIN
            ...
            IF AssemblyLine.FixedUsage THEN AssemblyLine.VALIDATE("Quantity per")
            ELSE
                AssemblyLine.VALIDATE("Quantity per", AssemblyLine."Quantity per" * QtyRatio);
            AssemblyLine.InitQtyToConsume;
        END;
        IF UpdateQtyToConsume THEN IF NOT AssemblyLine.FixedUsage THEN BEGIN
                AssemblyLine.InitQtyToConsume;
    ...
    procedure ShowAvailability(ShowPageEvenIfEnoughComponentsAvailable: Boolean; var TmpAssemblyHeader: Record "Assembly Header"; var TempAssemblyLine: Record "Assembly Line" temporary)Rollback: Boolean var
        Item: Record Item;
    ...
    begin
        IF TempNewAsmLine.FIND('-')THEN REPEAT TempNewAsmLine.SetSkipVerificationsThatChangeDatabase(FALSE);
    ...
    begin
        ...
        IF AssemblyLine.FIND('-')THEN BEGIN
            ...
            REPEAT AssemblyLine.SuspendStatusCheck(TRUE);
    ...
    local procedure LinesExist(AsmHeader: Record "Assembly Header"): Boolean var
        AssemblyLine: Record "Assembly Line";
    begin
        SetLinkToLines(AsmHeader, AssemblyLine);
        EXIT(NOT AssemblyLine.ISEMPTY);
    end;
    local procedure CalledFromRefreshBOM(ReplaceLinesFromBOM: Boolean; FieldNum: Integer): Boolean begin
    ...
    begin
        ...
        IF AssemblyLine.FINDSET THEN REPEAT LineAbleToAssemble:=CalcAvailToAssemble(AssemblyLine, AsmHeader, LineAvailabilityDate);
    ...
    local procedure CalcAvailToAssemble(AssemblyLine: Record "Assembly Line"; AsmHeader: Record "Assembly Header"; var LineAvailabilityDate: Date)LineAbleToAssemble: Decimal var
        Item: Record Item;
        GrossRequirement: Decimal;
    ...
    local procedure CalcEarliestDueDate(AsmHeader: Record 900; EarliestStartingDate: Date)EarliestDueDate: Date var
        ReqLine: Record 246;
    ...
    procedure GetNextAsmLineNo(var AsmLine: Record "Assembly Line"; AsmLineRecordIsTemporary: Boolean): Integer var
        TempAssemblyLine2: Record "Assembly Line" temporary;
    ...
    begin
        ...
        //--
        IF BOMComponent.FINDSET THEN REPEAT ComponentesEventoInsert.TRANSFERFIELDS(BOMComponent);
                ...
                IF BOMComponent."No." <> '' THEN BEGIN
                    ...
                    IF(ComponentesEventoInsert.Type = ComponentesEventoInsert.Type::Resource) AND (ComponentesEventoInsert."Resource Usage Type" = ComponentesEventoInsert."Resource Usage Type"::Fixed)THEN ComponentesEventoInsert.VALIDATE("Quantity per", ComponentesEventoInsert."Cantidad por Lote")
                    ELSE
                    BEGIN
                        IF ComponentesEvento."Cantidad por Lote" = 0 THEN ComponentesEventoInsert.VALIDATE("Quantity per", 0)
                        ELSE
                        BEGIN
    ...
    begin
        ...
        IF BOMComponent.FINDFIRST THEN BEGIN
            REPEAT AssemblyHeader.AddBOMLineEvento(BOMComponent);
    ...
    var
        ...
        ItemNo: Text;
    begin
        IF ComponentesEvento.FINDFIRST THEN REPEAT IF Item.GET(ComponentesEvento."No.")THEN BEGIN
                    Item.CALCFIELDS("Assembly BOM");
                    IF Item."Assembly BOM" THEN BEGIN
                        // Sumamos coste
